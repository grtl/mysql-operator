	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
				...
				return client.CrV1().MySQLBackups(namespace).List(options)
			},
			WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.CrV1().MySQLBackups(namespace).Watch(options)
			},
		},
		&cr_v1.MySQLBackup{},
		resyncPeriod,
...
func (f *mySQLBackupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&cr_v1.MySQLBackup{}, f.defaultInformer)
}
